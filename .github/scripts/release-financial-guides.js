...
const guides = [
  {
    ...
    indexNote: 'Best for readers who have a current statement and want to explain why a payoff balance is falling more slowly than expected. The guide ends with a monthly reconciliation record, not a generic promise to become debt-free.',
  ...
  },
  {
    ...
    indexNote: 'Use this before comparing listings or lender offers. It separates the payment a lender may approve from the full housing cost a household can maintain while preserving closing cash, repairs, and emergency reserves.',
  ...
  },
  {
    ...
    indexNote: 'Use this when a retirement result changes dramatically after one assumption moves. The checklist separates dated account and benefit evidence from planning preferences, then compares lower-return and timing scenarios one variable at a time.',
  ...
  },
  {
    ...
    indexNote: 'Use this to replace a generic months-of-expenses rule with a documented reserve range. It distinguishes predictable sinking funds from true emergencies and tests whether cash remains accessible when two shocks occur together.',
  ...
  },
  {
    ...
    indexNote: 'Use this when rent and principal-and-interest figures make one option look obviously cheaper. The guide adds transaction timing, repair exposure, alternative use of cash, and a flat-price case while keeping monthly affordability separate from projected net worth.',
...
const guideResearch = {
  'credit-card-payoff-mistakes': 'We reviewed the payoff workflow against the statement fields a cardholder can verify without sharing account credentials: balance category, APR, interest charge, minimum payment, fees, transactions, and payment dates. We then compared a fixed-payment case with a missed-extra-payment case and a continued-spending case. This check showed why the schedule must be reconciled after every statement instead of being treated as an issuer ledger. The guide therefore keeps promotional terms, balance categories, and new purchases visible as separate decision inputs.',
  'mortgage-affordability-guide': 'We reviewed the affordability workflow by separating lender qualification from household cash flow. The test record used the same price, down payment, term, and loan type across a base case and a higher-cost case, then added taxes, insurance, association dues, repairs, closing cash, and reserves outside principal and interest. This check showed that an apparently acceptable loan payment can still leave too little operating room. The guide therefore requires property-specific estimates and a written Loan Estimate before its result is used for a purchase decision.',
  'retirement-calculator-guide': 'We reviewed the retirement workflow by tracing every input to a dated source or an explicitly labeled planning choice. Current balances, contributions, employer rules, and the Social Security estimate were kept separate from return, inflation, spending, and retirement-age assumptions. We compared a base case with lower returns and a one-year timing change while holding other variables constant. This check showed which result changes came from new evidence and which came from preferences, so the guide requires a reproducible assumption record rather than a single target number.',
  'emergency-fund-planning': 'We reviewed the reserve workflow using essential expenses, deductibles, income-replacement time, access constraints, and known sinking funds as separate inputs. The test included a month with two simultaneous shocks and checked whether cash could be reached without market loss, a withdrawal penalty, or an avoidable transfer delay. This check showed that a generic months-of-expenses rule can miss fast, high-cost events or count predictable bills twice. The guide therefore uses a written withdrawal boundary, milestone targets, and a refill record after an actual emergency.',
  'rent-vs-buy-guide': 'We reviewed the comparison workflow across three holding periods while keeping housing assumptions visible: rent terms, loan terms, taxes, insurance, maintenance, purchase costs, sale costs, and the alternative use of cash. We also ran a flat-price case and separated monthly affordability from projected net worth. This check showed that a favorable long-term model can coexist with weak near-term cash flow or an unrealistic move date. The guide therefore treats flexibility, repair exposure, and transaction timing as decision evidence rather than hiding them inside one break-even result.'
};
...
const hubDefinitions = [
  {
    ...
    sections: [
      ...
      ['Close the loop after the month ends', 'Record actual income, essential spending, flexible spending, transfers, and unplanned costs beside the original plan. Explain the largest differences with statement evidence and decide whether each is temporary, recurring, or better handled by a separate sinking fund. Do not lower a savings target merely to make the month appear balanced. The purpose of reconciliation is to improve the next plan while preserving a clear record of what actually happened.']
    ],
  ...
  },
  {
    ...
    sections: [
      ...
      ['Define the walk-away conditions', 'Before making an offer, write down the maximum total monthly cost, minimum cash reserve after closing, acceptable repair exposure, and latest move date that still supports the holding-period assumption. Recheck those limits when the inspection, insurance indication, Loan Estimate, or association documents arrive. A decision that crosses a written limit needs new evidence and an explicit tradeoff, not a revised calculator input chosen only to preserve the deal.']
    ],
  ...
  },
  {
    ...
    sections: [
      ...
      ['Use differences as a diagnostic record', 'Classify a mismatch as wages, pay frequency, pre-tax deductions, after-tax deductions, federal withholding, state or local withholding, or payroll timing. Verify one category at a time against the pay statement and current forms. Keep the original estimate next to the corrected version so the source of the change remains visible. For filing status, credits, multiple jobs, or tax liability questions, use the official estimator or a qualified tax professional rather than treating this paycheck estimate as tax advice.']
    ],
...
function articleHtml(guide) {
  ...
  const research = guideResearch[guide.slug];
...
function blogIndex() {
  const cards = guides.map(g => `<a class="card" href="/blog/${g.slug}.html"><span>${g.category}</span><h2>${g.title}</h2><p>${g.description}</p><p>${g.indexNote}</p><strong>Read guide &rarr;</strong></a>`).join('\n');
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Financial Planning Guides | VestCalc</title><meta name="description" content="Five source-backed guides for using VestCalc debt, housing, savings, and retirement calculators responsibly."><meta name="robots" content="index, follow"><link rel="canonical" href="https://vestcalc.com/blog/"><link rel="icon" type="image/svg+xml" href="/favicon.svg"><meta property="og:title" content="Financial Planning Guides | VestCalc"><meta property="og:description" content="Five source-backed guides for using VestCalc debt, housing, savings, and retirement calculators responsibly."><meta property="og:url" content="https://vestcalc.com/blog/"><meta property="og:type" content="website"><meta name="twitter:card" content="summary_large_image"><style>*,*::before,*::after{box-sizing:border-box}body{margin:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;background:#f5f8fa;color:#172033;line-height:1.6}a{text-decoration:none;color:inherit}.portfolio-skip-link{position:fixed;left:12px;top:12px;z-index:30;background:#172033;color:#fff;padding:9px 12px;transform:translateY(-160%)}.portfolio-skip-link:focus{transform:none}header{background:#fff;border-bottom:1px solid #dbe4e8}.bar{max-width:1080px;margin:auto;min-height:64px;padding:10px 20px;display:flex;align-items:center;gap:18px}.brand{font-size:21px;font-weight:800}.bar nav{margin-left:auto;display:flex;gap:8px}.bar nav a{padding:7px 10px;color:#4f5d6b;font-weight:650}.hero{background:#0f766e;color:#fff}.hero div{max-width:1080px;margin:auto;padding:54px 20px}.hero h1{font-size:clamp(2rem,5vw,3.2rem);line-height:1.1;margin:0 0 12px}.hero p{max-width:760px;color:#e6fffb;font-size:18px;margin:0}main{max-width:1080px;margin:auto;padding:36px 20px 70px}.intro{max-width:780px;color:#4f5d6b;font-size:17px;margin-bottom:26px}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px}.card{min-width:0;background:#fff;border:1px solid #dbe4e8;padding:24px;transition:border-color .15s,transform .15s}.card:hover{border-color:#0d9488;transform:translateY(-2px)}.card span{font-size:12px;font-weight:800;text-transform:uppercase;color:#0f766e}.card h2{font-size:22px;line-height:1.28;margin:9px 0}.card p{color:#566176}.card strong{color:#0f766e}.standards{margin-top:28px;padding:22px;background:#e8f7f4;border-left:4px solid #0d9488}.standards h2{margin:0 0 12px;font-size:24px}.standards h3{font-size:18px;margin:22px 0 8px}.standards a{color:#0f766e;font-weight:750}.standards ul{padding-left:20px}.standards li{margin:8px 0}footer{background:#172033;color:#d5dde5;text-align:center;padding:28px 20px;font-size:13px}footer a{color:#99f6e4}.editorial-policy-link{display:inline;margin-left:10px}@media(max-width:700px){.grid{grid-template-columns:1fr}.bar{flex-wrap:wrap}.bar nav{width:100%;margin-left:0;overflow-x:auto}.hero div{padding:38px 18px}main{padding:28px 14px 48px}}</style></head><body><a class="portfolio-skip-link" href="#main-content">Skip to main content</a><header><div class="bar"><a class="brand" href="/">VestCalc</a><nav><a href="/">Calculators</a><a href="/blog/">Guides</a><a href="/methodology.html">Methodology</a></nav></div></header><div class="hero"><div><h1>Financial planning guides</h1><p>Focused explanations for five calculator workflows where assumptions, timing, and omitted costs can materially change the result.</p></div></div><main id="main-content"><p class="intro">Each released guide is reviewed for a distinct user question, links to primary public sources, and explains how to stress-test the related calculator. Broader draft topics remain out of search until they receive the same review.</p><div class="grid">${cards}</div><section class="standards"><h2>How to use this guide library</h2><p>Start with the decision you need to make, not the calculator with the largest result. Open the related guide, collect the documents it names, and record which inputs are confirmed, estimated, or still unknown. Then run a base case and one less favorable case without changing several assumptions at once. This process makes the difference between the two results explainable and gives you a record to revisit when a statement, quote, rate, or household condition changes.</p><h3>Evidence expected before a decision</h3><ul><li><strong>Debt:</strong> use the latest statement, balance category, APR, fees, payment dates, and promotional end date.</li><li><strong>Housing:</strong> use written rent terms or a Loan Estimate plus property-specific taxes, insurance, association costs, repairs, cash to close, and the expected holding period.</li><li><strong>Retirement:</strong> separate dated balances and plan rules from return, inflation, spending, and timing assumptions.</li><li><strong>Emergency savings:</strong> separate essential expenses, deductibles, predictable sinking funds, income-replacement time, and account-access limits.</li></ul><h3>Editorial review and corrections</h3><p>We reviewed each released workflow against the public documents a reader can verify without creating a VestCalc account or sending financial entries to us. Every guide identifies its primary public sources, names material limitations, and links to a calculator that can be reproduced with local inputs. We check that a less favorable scenario remains understandable and that the article does not present a planning estimate as an approval, forecast, or personalized recommendation.</p><p>Sources, product terms, laws, and household circumstances can change. The review date records when the page scope and source links were last checked; it does not guarantee that every external fact remains unchanged. Readers can report a factual or calculation issue through the <a href="/contact.html">contact page</a>. We preserve the decision context, correct the affected explanation or method, and update the review date when a material change is published.</p><p>VestCalc separates calculation methodology from editorial interpretation. Draft topics remain outside the public index until they meet the same evidence, scope, and correction standards. Reviewed by the VestCalc Editorial Team &middot; Updated ${updatedLabel}. Read the full <a href="/methodology.html">calculation methodology</a> and <a href="/editorial-policy.html">editorial and corrections policy</a>.</p></section></main><footer>&copy; 2026 VestCalc &middot; <a href="/about.html">About</a> &middot; <a href="/privacy.html">Privacy</a> &middot; <a href="/contact.html">Contact</a><p class="editorial-policy-link"><a href="/editorial-policy.html">Editorial Policy</a></p></footer></body></html>`;
}
