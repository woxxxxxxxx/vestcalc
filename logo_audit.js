...
const base='C:/Users/Administrator/vestcalc';
...
function collect(dir){
  try{
    fs.readdirSync(dir).forEach(function(f){
      ...
      if(s.isDirectory()&&f!=='node_modules'&&f!=='.git'&&f!=='.github'&&f!=='__pycache__') collect(fp);
...
const fileInfo=checkFiles.map(function(name){
  ...
  if(fs.existsSync(fp)){
    ...
    let content='';
    try{ content=fs.readFileSync(fp,'utf8').slice(0,200).replace(/\n/g,' '); }catch(e){ content='(binary)'; }
